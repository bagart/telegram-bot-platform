...
function buildTreeHtml(array $data, array $rows, array $totals): string
{
    ...
    $render = function (string $func, int $depth, array &$visited = []) use (&$render, &$tree, $totalWt, &$rows): string {
        ...
        $h = $indent . '<details><summary><span class="func-name">' . htmlspecialchars($func) . '</span> <span class="func-val">' . number_format($pct, 1) . '% / ' . number_format($wt / 1000, 2) . 'ms</span></summary>';
        if (isset($tree[$func])) {
